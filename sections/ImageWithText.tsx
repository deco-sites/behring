...
export default function ImageWithText(
  { subTitle, title, text, imagePosition, image, button, imageDecoration }:
    Props,
) {
  const commonCss =
  ...
  return (
    <div id="atuacao" class={imagePosition == "right" ? cssRight : cssLeft}>
      <div
        ...
        class={`animation max-w-[90vw] md:max-w-[55vw] flex items-center w-[90%] rounded-lg lg:w-full h-[380px] md:h-[550px] uw:h-[700px] overflow-hidden ${
          imagePosition == "right"
            ? "ml-auto rounded-tl-[400px] rounded-bl-[400px] mr-0 md:mr-[-60vw]"
            : "mr-auto rounded-tr-[400px] rounded-br-[400px] ml-0 md:ml-[-60vw]"
        }`}
      >
      ...
      <div
        ...
        class={`animation max-w-[740px] flex gap-10 md:w-[70%] ${
          imagePosition == "right"
            ? "flex-row-reverse ml-0 md:ml-[-50vw]"
            : "flex-row mr-0 md:mr-[-50vw]"
        }`}
      >
        <div class="flex px-[8vw] md:px-0 flex-col justify-start items-start px-[40px] md:px-[0px]">
          ...
          <a
            className="flex items-center bg-secondary px-[20px] rounded-[30px] uppercase text-white text-[12px] md:text-[16px] font-bold h-[40px] md:h-[50px]"
            href={button.link}
          >
            {button.text}
          </a>
        ...
        <div class={`hidden h-[380px] w-[140px] md:block `}>
          <img
            class="min-w-[70px] h-[380px] w-[140px]"
            src={imageDecoration}
            alt="Decoração - Fundação Behring"
          />
        </div>
      </div>
      <MidContentAnimation />
